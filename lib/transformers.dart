...
class ZoomInPageTransformer extends PageTransformer {
  ...
  Widget transform(Widget child, TransformInfo info) {
    ...
    if (position > 0 && position <= 1) {
      return Transform.translate(
        offset: Offset(-width * position, 0.0), // Changed here
        child: Transform.scale(
          scale: 1 - position,
          child: child,
        ),
...
class ZoomOutPageTransformer extends PageTransformer {
  static const double MIN_SCALE = 0.97;
  ...
  Widget transform(Widget child, TransformInfo info) {
    ...
    if (position < -1) {
      ...
      double vertMargin = 0.0; //pageHeight * (1 - scaleFactor) / 15;
  ...
  Widget transform(Widget child, TransformInfo info) {
    ...
    if (position <= 0) {
      ...
      return Opacity(
        ...
        child: Transform.translate(
          offset: Offset(
              0.0, 0.0), //-position * info.height! // info.width * -position
          child: Transform.scale(
            scale: scaleFactor,
            child: child,
          ),
